...
class Ender(serialDeviceSpec, Axes3DSpec):
    """Implemented interface for Lulzbot Taz 6 with BP modified firmware."""

    def __init__(self,
                 name='Ender',
                 posMode='relative',
                 devAddress="/dev/tty.usbserial-1110",
                 baudRate=115200,
                 commsTimeOut=1,
                 __verbose__=1,
                 firmwareVers='BP'):
    ...
    def sendCmd(self, command):
        ...
        time.sleep(1.5)   # give printer time to reply
    ...
    def handShakeSerial(self):
        ...
        try:
            for attempt in range(3):
                ...
                print(f"\t\tHandshake attempt {attempt + 1}, received:\n{readInput}")

                if readInput and any(keyword in readInput for keyword in ["FIRMWARE_NAME", "Marlin", "Creality"]):
                    print("\t\tFirmware response detected!")
                    return [1, "Handshake Success (Marlin/Creality Detected)"]

                print("\t\tNo valid firmware info found, retrying...")
                time.sleep(1)

            # If we reach here, printer responded but didn’t match known keywords
            return [0, "Handshake received but firmware version unrecognized"]

        except Exception as inst:
            import logging
            logging.exception(inst)
            return [-1, f"Error on Handshake: {inst}"]

        # try:
        #     readInput = self.sendCmd("M115\n")
        #     print("\t\tReceived handshake response:", readInput)

        #     if readInput is None:
        #         print("No response from printer — retrying handshake...")
        #         readInput = self.sendCmd("M115\n")

        #         if readInput is None:
        #             return [-1, "Error on Handshake: No response from serial device"]

        #     if "FIRMWARE_NAME" in readInput or "Marlin" in readInput:
        #         return [1, "Handshake Success (Marlin Detected)"]
        #     else:
        #         return [0, "Handshake received but firmware version unrecognized"]
        # except Exception as inst:
        #     import logging
        ...
        #     # Accept any valid M115 reply from the printer
        #     if "FIRMWARE_NAME" in readInput or "Marlin" in readInput:
        #         return [1, 'Handshake Success (Marlin Detected)']
        #     else:
        #         return [0, 'Handshake received but firmware version unrecognized']
